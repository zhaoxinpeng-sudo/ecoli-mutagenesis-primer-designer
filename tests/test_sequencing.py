...
from primer_designer.sequencing import RawRead, extract_sample_id, parse_seq, sample_id_key, verify_sample
...
def test_sample_id_and_seq_cleaning():
    ...
    assert extract_sample_id("A01_T7.ab1") == "A01"
    assert sample_id_key("a01") == sample_id_key("A01")
...
def test_synonymous_target_codon_passes_with_explanation():
    ...
    assert result.actual_amino_acid == result.expected_amino_acid == "V"
    assert "同义密码子" in result.reason
